...
Both values are also editable at run time in the **S3 Configuration** section of the
UI, which overrides the environment for that session.
...
4. **Process the Document**:
   ...
   - Click "Process All Samples" to batch process every bundle under `sample/`.
     ...
     roughly 30 pages per engine, and more once you add bundles of your own. The
     document and page count are logged before the first API call.
...
2. Uploaded as a `.json` file via **Upload Schema (.json)**. The file is validated
   ...
   practical route for claim-form schemas, which run to 12–17 KB.
...
| Cost                     | $0.040 per page (custom output) + $0.0005 per field beyond 30                                                                                                                                                                                                                  | $0.010 per page (standard output) + the structuring model's token cost                                                                   |
...
Two things the tooltip does not tell you:
...
- **Cost assumes one page.** Both `processor.py` and `BDAEngine.get_cost()` pass
  `page_count=1`, so BDA cost for a multi-page PDF is reported as the single-page
  figure regardless of the real page count.
...
- **BDA's boxes are real, but only on the blueprint path.** Geometry reaches the
  ...
  no-blueprint run on the seven-page sample returned 104 elements, none with a
  `locations` key. The service _can_ do better:
...
│   ├── pfl-synthetic.pdf               the document        (exactly one per bundle)
...
A fresh clone ships ten bundles: seven single images, and three multi-page insurance
claim forms named `*-synthetic`.

The claim forms are **generated, not scanned** — see
`tools/generate_synthetic_samples.py`. Every value in them is invented, which is what
makes a multi-page benchmark something the repository can ship and anyone can rerun:

```bash
python tools/generate_synthetic_samples.py            # regenerate the three bundles
python tools/generate_synthetic_samples.py --check    # report any drift, change nothing
```

Three properties of those samples are deliberate and are asserted by
`tests/test_synthetic_samples.py`:

- **No text layer.** Each page is drawn crisply, rasterised at 150 DPI with seeded
  speckle, a sub-degree skew and JPEG compression, and the crisp page is then
  discarded. Claim forms arrive as fax-grade scans, so the engines have to genuinely
  OCR these — a text layer would let Bedrock and BDA read the values straight out of
  the file, and every accuracy and cost figure would describe a different task.
- **Ground truth by construction.** `truth.json` is written from the same value table
  the pages are drawn from, so it cannot drift from the document. Editing a value means
  regenerating; `--check` is what tells you that you forgot.
- **Deterministic.** Rendering is seeded and PDF metadata is cleared, so regenerating
  does not churn a few hundred kilobytes of binary on every run.

They also cover the schema shapes the BDA blueprint translation has to survive — a
group inside a group, an array of objects inside a group, and an optional numeric
column — which is what `tests/test_blueprint_schema.py` reads them for.
...
Measured on `sample/pfl-synthetic/` (4 pages) with the UI defaults: Textract 98.68%,
Bedrock 98.68%, BDA 98.68%, and BDA with a custom blueprint 100.0%.
...
sub-directory per sample and per engine holding that engine's extracted text
(`text.txt`), structured JSON (`result.json`), annotated visualisation, and
`metadata.json`; plus a `summary.json` for the batch. The summary's per-engine
figures are `documents_processed`, `total_pages`, `avg_time_per_document`,
`avg_time_per_page`, `avg_cost_per_document`, `avg_cost_per_page` and
`avg_accuracy` — the per-page entries are `null` when no page count was reported.
